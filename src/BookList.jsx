  ...
  if (books.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-md text-center">
        ...
        <p className="mt-1 text-gray-500">Add your first book using the form.</p>
      </div>
    );
  }
  return (
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 whitespace-nowrap text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Publication Date</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {books.map((book) => (
              <tr key={book.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(book.publish_date).toLocaleDateString("id-ID")}</td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{book.publisher}</td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                  <div className="flex space-x-2">
                    <button onClick={() => onEdit(book)} className="p-1 text-blue-600 hover:text-blue-800" title="Edit">
                      <PencilIcon className="w-5 h-5" />
                    </button>
                    <button onClick={() => onDelete(book.id)} className="p-1 text-red-600 hover:text-red-800" title="Delete">
